function Efficiency=efficiencyFunction(Transport,Consumers,Producers)
...
for i=1:size(Producers.coordinates,1)
    for j=1:size(Consumers.coordinates,1)
         Efficiency(i,j)=Transport.efficiency^...
             distanceFunction(Producers.coordinates(i,:),Consumers.coordinates(j,:));
    end
end
end